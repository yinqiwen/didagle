...
void GraphDataContext::SetArena(
...
DataValue *GraphDataContext::GetValue(const DIObjectKeyView &key,
  ...
  if (opt.with_parent && _parent) {
    ...
    if (new_excludes->count(_parent) == 0) {
      r = const_cast<GraphDataContext *>(_parent)->GetValue(key, parent_opt,
                                                            new_excludes);
    }
    if (r) {
      return r;
  ...
  if (opt.with_children) {
    ...
    for (const GraphDataContext *child_ctx : _executed_childrens) {
      if (nullptr != child_ctx && new_excludes->count(child_ctx) == 0) {
        r = const_cast<GraphDataContext *>(child_ctx)->GetValue(key, child_opt,
                                                                new_excludes);
        if (r) {
          return r;
        }
...
size_t Processor::RegisterParam(const std::string &name,
...
// int Processor::InjectInputField(GraphDataContext& ctx, const std::string&
// field_name, const std::string_view& data_name,
//                                 bool move) {
...
//   return rc;
// }
// int Processor::EmitOutputField(GraphDataContext& ctx, const std::string&
// field_name,
//                                const std::string_view& data_name) {
//   auto found = _field_emit_table.find(field_name);
//   if (found == _field_emit_table.end()) {
//     DIDAGLE_DEBUG("[{}]EmitOutputField field:{} not found", Name(),
//     field_name); return -1;
//   }
...
void ProcessorFactory::Register(std::string_view name,
...
ProcessorRegister::ProcessorRegister(std::string_view name,
                                     const ProcessorCreator &creator) {
