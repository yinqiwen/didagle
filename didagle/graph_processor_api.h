...
class GraphDataContext {
public:
  using ExcludeGraphDataContextSet =
      folly::F14ValueSet<const GraphDataContext *>;

private:
  GraphDataContext() = default;
  using DataValuePtr = std::unique_ptr<DataValue>;
  using DataTable =
      folly::F14FastMap<DIObjectKeyView, DataValuePtr, DIObjectKeyViewHash,
  ...
  inline const DataValue *GetDataValue(const DIObjectKeyView &key,
                                       int32_t idx) const {
    if (idx >= 0) {
...
public:
  ...
  void Reset();
  template <typename T>
  inline typename DIObjectTypeHelper<T>::read_type
  Get(const std::string_view &name, int32_t idx = -1,
  ...
  template <typename T>
  inline typename DIObjectTypeHelper<T>::read_write_type
  Move(const std::string_view &name, int32_t idx = -1,
       GraphDataGetOptions opt = {},
       ExcludeGraphDataContextSet *excludes = nullptr) {
    ...
    if (opt.with_parent && _parent) {
      ...
      if (new_excludes->count(_parent) == 0) {
        MoveValueType r = const_cast<GraphDataContext *>(_parent)->Move<T>(
            name, -1, parent_opt, new_excludes);
        if (r) {
          return r;
        }
      }
...
using InjectFunc = std::function<int(GraphDataContext &, int32_t,
                                     const std::string_view &, bool)>;
using EmitFunc =
    std::function<int(GraphDataContext &, int32_t, const std::string_view &)>;
...
protected:
  ...
  // bool)> InjectFunc; typedef std::function<int(GraphDataContext&, const
  // std::string_view&)> EmitFunc;
  ...
  virtual Awaitable<int> OnCoroExecute(const Params &args) {
    co_return ERR_UNIMPLEMENTED;
  }
#endif
  virtual folly::Future<int> OnFutureExecute(const Params &args) {
    return folly::makeFuture<int>(ERR_UNIMPLEMENTED);
  }
  virtual AdaptiveWait<int> OnAExecute(const Params &args) {
    co_return ERR_UNIMPLEMENTED;
  }
...
public:
  ...
  GetString(GetStringMode mode = GetStringMode::kDefault) const {
    return "";
  }
...
#if ISPINE_HAS_COROUTINES
  Awaitable<int> CoroExecute(const Params &args);
#endif
  AdaptiveWait<int> AExecute(const Params &args);
...
#define __GRAPH_OP_INPUT(TYPE, NAME, FLAGS)                                    \
  ...
  size_t __input_##NAME##_code = RegisterInput<BOOST_PP_REMOVE_PARENS(TYPE)>(  \
      ...
      BOOST_PP_STRINGIZE(BOOST_PP_REMOVE_PARENS(TYPE)),                                       \
          [this](didagle::GraphDataContext &ctx, int32_t idx,                  \
                 const std::string_view &data, bool move) {                    \
  ...
  size_t __input_##NAME##_code = RegisterInput<BOOST_PP_REMOVE_PARENS(TYPE)>(  \
      #NAME, BOOST_PP_STRINGIZE(BOOST_PP_REMOVE_PARENS(TYPE)),                                \
                 [this](didagle::GraphDataContext &ctx, int32_t idx,           \
                   ...
                   typename didagle::DIObjectTypeHelper<FIELD_TYPE>::read_type \
                       tmp = {};                                               \
                   if (FOLLY_LIKELY(!move)) {                                  \
...
#define GRAPH_OP_ARENA_OUTPUT(TYPE, NAME)                                      \
  ...
  size_t __prepare_##NAME##_code = AddPrepareFunc([this]() {                   \
    NAME =                                                                     \
        google::protobuf::Arena::CreateMessage<BOOST_PP_REMOVE_PARENS(TYPE)>(  \
            GetArena());                                                       \
  });
...
#define GRAPH_PARAMS_string(name, val, txt)                                    \
  ...
  size_t __PARAMS_##name##_code = RegisterParam(BOOST_PP_STRINGIZE(name), "string", val, txt, [this](const didagle::Params &args) {         \
...
#define GRAPH_PARAMS_string_vector(name, val, txt)                             \
  ...
  size_t __PARAMS_##name##_code = RegisterParam(BOOST_PP_STRINGIZE(name), "vector<string>",                                                 \
      BOOST_PP_STRINGIZE(val), txt, [this](const didagle::Params &args) {      \
...
#define GRAPH_PARAMS_int(name, val, txt)                                       \
  ...
  size_t __PARAMS_##name##_code = RegisterParam(BOOST_PP_STRINGIZE(name), "int",                                                            \
      BOOST_PP_STRINGIZE(val), txt, [this](const didagle::Params &args) {      \
...
#define GRAPH_PARAMS_int_vector(name, val, txt)                                \
  ...
  size_t __PARAMS_##name##_code = RegisterParam(BOOST_PP_STRINGIZE(name), "vector<int>",                                                    \
      BOOST_PP_STRINGIZE(val), txt, [this](const didagle::Params &args) {      \
        if (args.Contains(BOOST_PP_STRINGIZE(name))) {                         \
          PARAMS_##name.clear();                                               \
          const auto &member = args[BOOST_PP_STRINGIZE(name)];                 \
...
#define GRAPH_PARAMS_bool(name, val, txt)                                      \
  ...
  size_t __PARAMS_##name##_code = RegisterParam(BOOST_PP_STRINGIZE(name), "bool",                                                           \
      BOOST_PP_STRINGIZE(val), txt, [this](const didagle::Params &args) {      \
...
#define GRAPH_PARAMS_double(name, val, txt)                                    \
  ...
  size_t __PARAMS_##name##_code = RegisterParam(BOOST_PP_STRINGIZE(name), "double",                                                         \
      BOOST_PP_STRINGIZE(val), txt, [this](const didagle::Params &args) {      \
...
#define GRAPH_PARAMS_double_vector(name, val, txt)                             \
  ...
  size_t __PARAMS_##name##_code = RegisterParam(BOOST_PP_STRINGIZE(name), "vector<double>",                                                 \
      BOOST_PP_STRINGIZE(val), txt, [this](const didagle::Params &args) {      \
        if (args.Contains(BOOST_PP_STRINGIZE(name))) {                         \
          PARAMS_##name.clear();                                               \
          const auto &member = args[BOOST_PP_STRINGIZE(name)];                 \
