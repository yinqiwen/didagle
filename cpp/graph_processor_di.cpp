    ...
    if (nullptr != graph_data && !graph_data->aggregate.empty()) {
        ...
        if (!aggregate_id.empty() && aggregate_id[0] == '$' && nullptr != params) {
          ...
          if (!var_value.String().empty()) {
            ...
            if (required)
              ...
              DIDAGLE_DEBUG("[{}]inject {} failed with var aggregate_id:{}", _proc->Name(), field,
                            aggregate_id);
          }
        }
      ...
      if (!data.name.empty() && data.name[0] == '$' && nullptr != params) {
        ...
        if (!var_value.String().empty()) {
          ...
          if (required) {
            ...
            DIDAGLE_DEBUG("[{}]inject {} failed with var data name:{}", _proc->Name(), field,
                          data.name);
          }
        }
      } else {
      ...
      if (0 != rc) {
        if (required) {
          ...
          DIDAGLE_DEBUG("[{}]inject {}:{} failed with move:{}, null data:{}", _proc->Name(), field,
                        data.name, move_data, nullptr == graph_data);
        }
      }
    }
